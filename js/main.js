...
function openAlbum(albumId) {
    ...
    setTimeout(() => {
        ...
        injectLayoutToggleUI(currentAlbum);
...
// ========================================
// APPLY PHOTO ASPECT RATIO
// ========================================

function applyPhotoAspect(layoutMode) {
  const m = (layoutMode === 'portrait') ? 'portrait' : 'landscape'; // default landscape
  const aspect = (m === 'portrait') ? '4 / 5' : '16 / 9';
  document.documentElement.style.setProperty('--photo-aspect', aspect);
  console.log('[MAIN] --photo-aspect =>', aspect, 'mode=', m);
}

function scrollToPhotoTop() {
  const gallery = document.getElementById('photo-gallery');
  if (!gallery) return;

  // galleryの先頭が画面上部に来るように
...
// ========================================
// LAYOUT TOGGLE UI INJECTION
// ========================================

function injectLayoutToggleUI(album) {
  const gallery = document.getElementById('photo-gallery');
  if (!gallery) return;

  // 二重挿入防止
  document.getElementById('layout-toggle')?.remove();

  // 初期：未指定なら landscape
  const initialMode = (album?.layoutMode === 'portrait') ? 'portrait' : 'landscape';
 
  const wrap = document.createElement('div');
  wrap.id = 'layout-toggle';
  wrap.className = 'layout-toggle';

  wrap.innerHTML = `
    <button class="layout-icon-btn" type="button" data-mode="landscape" aria-label="Landscape">
      <span class="layout-rect layout-rect-landscape" aria-hidden="true"></span>
    </button>

    <button class="pc-toggle" type="button" role="switch" aria-label="Toggle layout">
      <span class="pc-toggle-knob" aria-hidden="true"></span>
    </button>

    <button class="layout-icon-btn" type="button" data-mode="portrait" aria-label="Portrait">
      <span class="layout-rect layout-rect-portrait" aria-hidden="true"></span>
    </button>
  `;

  const switchBtn = wrap.querySelector('.pc-toggle');
  const leftBtn = wrap.querySelector('.layout-icon-btn[data-mode="landscape"]');
  const rightBtn = wrap.querySelector('.layout-icon-btn[data-mode="portrait"]');

  const setMode = (nextMode, opts = { scroll: false }) => {
    const m = (nextMode === 'portrait') ? 'portrait' : 'landscape';
    album.layoutMode = m;

    // 見た目の唯一のソース：必ず片方だけ付く
    wrap.classList.remove('is-portrait', 'is-landscape');
    wrap.classList.add(m === 'portrait' ? 'is-portrait' : 'is-landscape');

    // ノブ右＝portrait（aria-checked=true を portrait に割当）
    switchBtn.setAttribute('aria-checked', String(m === 'portrait'));

    // 比率反映
    applyPhotoAspect(m);

    // 切替後：写真先頭へ（タイトルではなく写真エリア）
    if (opts.scroll) setTimeout(scrollToPhotoTop, 0);

    console.log('[MAIN] layoutMode =>', m, 'wrap=', wrap.className);
  };
  ...
  // アイコン押下
  leftBtn.addEventListener('click', () => setMode('landscape', { scroll: true }));
  rightBtn.addEventListener('click', () => setMode('portrait', { scroll: true }));

  // トグル押下で反転
  switchBtn.addEventListener('click', (e) => {
    e.preventDefault();
    const isPortrait = switchBtn.getAttribute('aria-checked') === 'true';
    setMode(isPortrait ? 'landscape' : 'portrait', { scroll: true });
  });
